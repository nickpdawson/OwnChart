...
- **Acceptance test results** for each of the 20 items in §13, run against a real iPhone with OwnChart build 43+ (build 43 adds the Review Health permissions action on the MCP Settings screen and the canonical read-scope set shared by direct sync and MCP — PM-accepted 2026-05-30 for Beta 1.1; build 42 pins the listener to static port `52121`, removes `authorizationStatus(for:)` pre-filtering, and flattens the capabilities shape to per-identifier `available`/`unknown` status, plus keeps the iPhone awake + server alive while the MCP Settings screen is foreground; build 41 added sleep / workouts to `query_daily_summary`; build 40 introduced stable `server_id` + Bonjour TXT records, client-metadata in `/pair`, and the pairing-doesn't-stop-server invariant; build 39 brought background grace and persistent pairing). Build 43 also surfaced the non-blocking medication-dose follow-up tracked in `Working Docs/FU-HK-MCP-MEDICATION-DOSE-EVENTS.md`.
