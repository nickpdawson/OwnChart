...
- [HealthKit MCP bridge](./user-docs/HEALTHKIT_MCP.md) — let Claude Desktop / Claude Code / Codex read your iPhone HealthKit data over Wi-Fi via a small open-source bridge. macOS + Node 20+, local-only (no cloud relay, no OwnChart backend in the path), and the iOS app has to be running when you query. npm: [`ownchart-hk-mcp-bridge`](https://www.npmjs.com/package/ownchart-hk-mcp-bridge); source: [github.com/nickpdawson/ownchart-hk-mcp-bridge](https://github.com/nickpdawson/ownchart-hk-mcp-bridge).
